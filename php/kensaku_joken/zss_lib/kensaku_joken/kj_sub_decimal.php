 ...
 *
 */
class KjSubDecimal implements IKjSub{
    ...
    public function getDefInpData(){
    	...
    	$data['size']=DECIMAL_LEN;					//入力文字数
    	$data['primaryKey']=false;//主キー
    	$data['req']=false;				//必須入力
    	$data['maxvalue']=DECIMAL_MAX;			//数値入力系の場合の最大値
    	$data['minvalue']=DECIMAL_MIN;			//数値入力系の場合の最小
    ...
    public function createJokenSql($kjEnt){
    	//検索条件SQL文を生成する。数値系用。
    	$util=KjUtil::getInstance();
			$html=$util->createJokenSqlForNum(&$kjEnt);
